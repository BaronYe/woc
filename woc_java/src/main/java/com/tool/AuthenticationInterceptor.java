...
public class AuthenticationInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, Object object) throws Exception {
        ...
        if (method.isAnnotationPresent(Token.LoginToken.class)) {
            Token.LoginToken userLoginToken = method.getAnnotation(Token.LoginToken.class);
            if (userLoginToken.required()) {
                // 执行认证
                if (token == null) {
                    throw new MyException(ExceptionEnum.TOKEN_NULL);
                }
                // 获取 token 中的 user id
                String account;
                try {
                ...
                if (admin == null) {
                    throw new MyException(ExceptionEnum.ADMIN_NULL);
                }
                // 验证 token
                JWTVerifier jwtVerifier = JWT.require(Algorithm.HMAC256(String.valueOf(admin.getAccount()))).build();
                try {
