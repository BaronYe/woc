    ...
    <template>
      <el-table
        ...
        <el-table-column
            ...
            label="商品主图">
          <template slot-scope="scope">
            <el-image :src="scope.row.integralGoodsPicture" :preview-src-list="[scope.row.integralGoodsPicture]"
  ...
  methods: {
    ...
    delNsOderById(id) {
      this.$confirm('此操作将删除该订单, 是否继续?', '提示', {
        ...
        this.api.delIntegralOrderById({ id }).then(res => {
          this.$message.success(res.msg);
          setTimeout(_=>{
            this.getIntegralOrderFn();
          },1500)
