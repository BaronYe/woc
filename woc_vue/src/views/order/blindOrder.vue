          ...
          <el-form label-position="right" label-width="120px">
            <div class="d-flex ai-center">
              ...
              <el-form-item label="出售类型：">
                <el-select v-model="isSellState" placeholder="全部">
                  <el-option
                      v-for="item in isSellStateList"
                      :key="item.id"
                      :label="item.name"
                      :value="item.id">
                  </el-option>
            ...
            <div class="d-flex ai-center">
              ...
              <el-form-item label="订单类型：">
                <el-select v-model="isTuangou" placeholder="全部">
                  <el-option
                      v-for="item in isTuangouList"
                      :key="item.id"
                      :label="item.name"
                      :value="item.id">
                  </el-option>
    ...
    <template>
      <el-table
          ...
          <template slot-scope="scope">
            <div style="cursor: pointer;" v-if="scope.row.item != ''">
              <el-popover trigger="hover" placement="top">
                ...
                <p>商品图片：<el-image :src="scope.row.item.spescImg" :preview-src-list="[scope.row.item.spescImg]"
                                  style="width: 30px;height: 30px;"></el-image></p>
        ...
        <el-table-column
            ...
            label="订单类型">
          <template slot-scope="scope">
            <div v-if="scope.row.isTuangou === 0">正常订单</div>
            <div v-else>拼团订单</div>
          </template>
        </el-table-column>
        <el-table-column
            ...
            label="订单金额/积分">
          <template slot-scope="scope">
            <div v-if="scope.row.paymentType === 2">{{ scope.row.point }}积分</div>
            <div v-else>¥{{ scope.row.orderMoney }} 元</div>
          </template>
        </el-table-column>
        <el-table-column
            align="center"
        ...
        <el-table-column
            ...
            label="出售状态">
          <template slot-scope="scope">
            <div v-if="scope.row.isSellState ===  1" style="display: flex;align-items: center;justify-content: center;cursor: pointer;">
              邮寄
              <el-popover trigger="hover" placement="top">
                <div>
                ...
                <div slot="reference" class="name-wrapper">
                  <div style="color: #20a0ff;margin-left: 10px;">地址</div>
                </div>
              </el-popover>
            </div>
            <div v-else-if="scope.row.isSellState ===  2">出售</div>
            <div v-else-if="scope.row.isSellState ===  3">暂存</div>
            <div v-else>未选</div>
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-button
                @click="comDialogClick(scope.row.orderId)"
                v-if="scope.row.orderStatus === 1 &&  scope.row.isSellState == 1"
                type="text"
                size="small">
              发货
            </el-button>
            <el-button
                @click="takeClick(scope.row.orderId)"
                v-if="scope.row.orderStatus === 2 &&  scope.row.isSellState == 1"
                type="text"
                size="small">
              确认收货
            </el-button>
            <el-button
                @click.native.prevent="delNsOderById(scope.row.orderId)"
                type="text"
                size="small">
              删除
            </el-button>
  ...
  methods: {
    ...
    getNsOrderFn() {
      this.api.getNsOrder({
        ...
        isSellState: this.isSellState,
        isTuangou: this.isTuangou,
      }).then(res => {
    ...
    delNsOderById(id) {
      this.$confirm('此操作将删除该订单, 是否继续?', '提示', {
        ...
        this.api.delNsOderById({ orderId:id }).then(res => {
          this.$message.success(res.msg);
          setTimeout(_=>{
            this.getNsOrderFn();
          },1500)
