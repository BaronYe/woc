    ...
    <el-drawer
      ...
      <div>
        <div class="d-flex ai-center">
          <el-button class="simon-button" @click="addCircle">添加商圈</el-button>
        </div>
        <!-- 列表 -->
        <template>
          <el-table
      ...
      <div class="simon-drawer__content">
        <el-form label-position="right" :model="form" :rules="rules" ref="formRef" v-if="form != null"
          ...
          <el-form-item label="商圈名称" prop="name">
            <el-input v-model="form.name" placeholder="请填写商圈名称"></el-input>
          </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      form:null,
      rules:{
        name: [{ required: true, message: '请填写商圈名称' }],
        img: [{ required: true, message: '请上传商圈底图' }],
  ...
  methods: {
    ...
    setClick(formName){
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.api.setCircle(this.form).then(res=>{
    ...
    delClick(id){
      this.$confirm('此操作将删除该活动, 是否继续?', '提示', {
        ...
        this.api.delCircleById({id}).then(res=>{
